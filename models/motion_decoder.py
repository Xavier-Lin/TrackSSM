...
class Time_info_decoder(nn.Module):
    ...
    def forward(self, x_0, flow_features, h=None, curr_gt = None): # 输入的x为condition 也就是 轨迹的 历史帧的位置信息 和 运动信息 b 5 8 ---- 默认 使用的是 历史帧的5帧
        # import pdb;pdb.set_trace()
        b, p_dim = x_0.shape
        x_inital = x_0
        inter_pred_bboxes = []
        ...
        if self.training:
            ...
            for n, pred in enumerate(all_pred_bboxes):
                ...
                #                     box_cxcywh_to_xyxy(curr_gt.view(-1, p_dim))
                #                 )
                #             )
