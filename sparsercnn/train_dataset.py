...
class MOTtrainset(data.Dataset):
    ...
    def randshift(self, img, targets):
        self.xshift = 50 * np.random.rand(1) # 返回bs个 0-100之间的随机数 对应bs个图像的x方向的随机变化offset 
        self.xshift *= (np.random.randn(1) > 0.0) * 2 - 1 # 如果是T 则不变+ 如果是F 则 xshift为 -
        self.yshift = 50 *np.random.rand(1) # 返回bs个 0-100之间的随机数 对应bs个图像的y方向的随机变化offset
    ...
    def aug_imgs_and_targets(self, data_transforms, image, anns):
        ...
        aug_anns = [
            utils.transform_instance_annotations(obj, transforms_pre, image_shape,
                                                  is_clip=False
